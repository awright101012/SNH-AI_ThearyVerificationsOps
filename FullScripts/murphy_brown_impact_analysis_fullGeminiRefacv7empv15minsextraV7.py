...
# -*- coding: utf-8 -*-
"""
murphy_brown_impact_analysis_fullGeminiRefacv7empv15minsextraV7
...
def load_history_from_csv(file_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        # Standardize potential office name variations (e.g., 'officename' to 'office')
...
def perform_mb_impact_analysis(df_history):
    ...
    print("Step 5b: Calculating and saving monthly completion breakdowns to Excel...")
    monthly_overall = pd.DataFrame() # Initialize empty
    monthly_office = pd.DataFrame()
    monthly_agent = pd.DataFrame()
    
    if 'is_completed' in df_summary.columns and 'completiondate' in df_summary.columns and pd.api.types.is_datetime64_any_dtype(df_summary['completiondate']):
        ...
        if not completed.empty and completed['completiondate'].notna().any():
            ...
            monthly_overall = (
                completed
                .groupby('month_period')['searchid']
                .count()
                .reset_index(name='completions')
            )

            # 3) by office (if you carried 'office' into df_summary)
            if 'office' in completed.columns:
                monthly_office = (
                    completed
                    .groupby(['month_period', 'office'])['searchid']
                    .count()
                    .reset_index(name='completions')
                )
            else:
                print("Warning: 'office' column not found in summary for monthly Excel breakdown.")
                monthly_office = pd.DataFrame(columns=['month_period','office','completions'])

            # 4) by agent (ensure agentname exists)
            if 'agentname' in completed.columns:
                monthly_agent = (
                    completed
                    .groupby(['month_period', 'agentname'])['searchid']
                    .count()
                    .reset_index(name='completions')
                )
            else:
                print("Warning: 'agentname' column not found in summary for monthly Excel breakdown.")
                monthly_agent = pd.DataFrame(columns=['month_period','agentname','completions'])

            # 5) print to terminal
            if not monthly_overall.empty:
                print("=== Monthly Completions: Overall ===")
                print(monthly_overall.to_string(index=False))
            if not monthly_office.empty:
                print("=== Monthly Completions: By Office ===")
                print(monthly_office.to_string(index=False))
            if not monthly_agent.empty:
                print("=== Monthly Completions: By Agent ===")
        ...
        print("Warning: Missing 'is_completed' or 'completiondate' (or invalid type) in summary. Skipping monthly Excel breakdown.")
    # --- END NEW Excel Block ---
    ...
    print("\n--- Creating Detailed Sample for Verification (Completed Only, 15 days ending 1 week prior to max date) ---") # Updated print statement
    if 'historydatetime' in df_history.columns and pd.api.types.is_datetime64_any_dtype(df_history['historydatetime']) and 'is_completed' in df_summary.columns:
        max_hist_date = df_history['historydatetime'].max()
        if pd.notna(max_hist_date):
            # --- MODIFICATION START ---
            # Calculate the new date range
            end_date_15_day = max_hist_date - pd.Timedelta(days=7)
            start_date_15_day = end_date_15_day - pd.Timedelta(days=15)
            print(f"Filtering history for detailed sample (Date Range: {start_date_15_day.date()} to {end_date_15_day.date()})")

            # 1. Identify completed search IDs from the summary
            completed_search_ids = df_summary.loc[df_summary['is_completed'] == 1, 'searchid'].unique()

            if len(completed_search_ids) > 0:
                # 2. Filter history for the calculated 15-day range AND only for completed search IDs
                df_history_15_day_completed = df_history[
                    (df_history['historydatetime'] >= start_date_15_day) &
                    (df_history['historydatetime'] <= end_date_15_day) & # Ensure it's within the end date
                    df_history['historydatetime'].notna() &
                    df_history['searchid'].isin(completed_search_ids) # Filter by completed IDs
                ].copy()

                if not df_history_15_day_completed.empty:
                    # 3. Merge with the final summary dataframe
                    summary_cols_to_merge = [col for col in df_summary.columns if col != 'searchid']
                    df_verification_sample = pd.merge(
                        df_history_15_day_completed, # Use the newly filtered history
                        df_summary[['searchid'] + summary_cols_to_merge],
                        on='searchid',
                        how='left',
                        suffixes=('_hist', '')
                    )
                    print(f"Created detailed verification sample (completed, 15-day period) with {len(df_verification_sample)} rows for {len(df_history_15_day_completed['searchid'].unique())} searches.")
                    # Update filename to reflect the new period
                    save_results(df_verification_sample, "detailed_sample_15d_completed_only")
                else:
                    print(f"Warning: No history records found for completed searches within the specified 15-day period ({start_date_15_day.date()} to {end_date_15_day.date()}).")
            else:
                print("Warning: No completed searches found in the summary table.")
            # --- MODIFICATION END ---
        else:
            print("Warning: Could not determine max history date for detailed sample.")
    else:
        print("Warning: 'historydatetime' column missing/invalid in df_history or 'is_completed' missing in df_summary. Cannot create 15-day completed sample.")
