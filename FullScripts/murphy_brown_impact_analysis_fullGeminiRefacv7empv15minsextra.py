...
def perform_mb_impact_analysis(df_history):
    ...
    print("\n--- Creating Detailed Sample for Verification (Last 30 Days, Completed Only) ---") # Updated print statement
    if 'historydatetime' in df_history.columns and pd.api.types.is_datetime64_any_dtype(df_history['historydatetime']) and 'is_completed' in df_summary.columns:
        ...
        if pd.notna(max_hist_date):
            ...
            if len(completed_search_ids) > 0:
                # 2. Filter history for the last 30 days AND only for completed search IDs
                df_history_last_30_days_completed = df_history[
                    (df_history['historydatetime'] >= start_date_30_days) &
                    df_history['historydatetime'].notna() &
                    df_history['searchid'].isin(completed_search_ids) # Filter by completed IDs
                ].copy()

                if not df_history_last_30_days_completed.empty:
                    # 3. Merge with the final summary dataframe (only relevant summary columns needed)
                    summary_cols_to_merge = [col for col in df_summary.columns if col != 'searchid']
                    df_verification_sample = pd.merge(
                        df_history_last_30_days_completed, # Use the filtered history
                        df_summary[['searchid'] + summary_cols_to_merge],
                        on='searchid',
                        how='left',
                        suffixes=('_hist', '') # Suffix original history cols if they overlap
                    )
                    print(f"Created detailed verification sample (completed only) with {len(df_verification_sample)} rows for {len(completed_search_ids)} searches.")
                    save_results(df_verification_sample, "detailed_sample_last_30d_completed_only") # Updated filename
                else:
                    print("Warning: No history records found for completed searches within the last 30 days.")
            else:
        ...
        print("Warning: 'historydatetime' column missing/invalid in df_history or 'is_completed' missing in df_summary. Cannot create 30-day completed sample.")
