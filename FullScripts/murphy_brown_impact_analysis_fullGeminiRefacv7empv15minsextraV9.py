        ...
        if int_col in completed_df.columns and out_col in completed_df.columns:
            ...
            for row in ys.itertuples():
                if row.attempts > 0:
                    ...
                    mb_status = "MB" if row.mb_touched == 1 else "non-MB"
                    print(f"{ch.capitalize()} channel yield for {mb_status}: 0.00% (0/0 attempts)")
...
def perform_mb_impact_analysis(df_history):
    ...
    print(f"Merged TTV block. Summary table shape (after TTV merge): {df_summary.shape}")
    ...
    print("Step 3b: Adding fax/phone interaction and method counts...")
    for ch in ['fax', 'phone']:
    ...
    print("Step 5b: Calculating and saving monthly completion breakdowns by searchtype to Excel...") # Modified print statement
