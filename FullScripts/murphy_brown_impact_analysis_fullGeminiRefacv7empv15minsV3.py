...
def perform_mb_impact_analysis(df_history):
    ...
    # --- Create Detailed Sample Output (15 days, ending 7 days prior, completed only) ---
    print("\n--- Creating Detailed Sample for Verification (15 Days, Completed Only) ---")
    if ('historydatetime' in df_history.columns and pd.api.types.is_datetime64_any_dtype(df_history['historydatetime']) and 
        'is_completed' in df_summary.columns):
        ...
        if pd.notna(max_hist_date):
            ...
            print(f"Filtering history for detailed sample between {start_date_15_day.date()} and {end_date_15_day.date()} (Completed Searches Only)")
            
            # 1. Filter history by the date range
            df_history_in_range = df_history[
                (df_history['historydatetime'] >= start_date_15_day) & 
                (df_history['historydatetime'] <= end_date_15_day) &
                df_history['historydatetime'].notna()
            ].copy()

            # 2. Get IDs of searches that were completed
            completed_search_ids = df_summary.loc[df_summary['is_completed'] == 1, 'searchid'].unique()
            
            # 3. Filter the in-range history to only include completed search IDs
            df_history_sample_base = df_history_in_range[
                df_history_in_range['searchid'].isin(completed_search_ids)
            ]

            if not df_history_sample_base.empty:
                # 4. Merge with the final summary dataframe
                summary_cols_to_merge = [col for col in df_summary.columns if col != 'searchid']
                df_verification_sample = pd.merge(
                    df_history_sample_base,
                ...
                print("Warning: No completed searches found within the specified 15-day date range for detailed sample.")
        else:
            print("Warning: Could not determine max history date for detailed sample.")
    else:
        print("Warning: 'historydatetime' or 'is_completed' columns missing or invalid. Cannot create 15-day completed sample.")
